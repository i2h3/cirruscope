...
enum EntityActivation {
    ...
    static func outcome(forConversationToken token: String, in store: AccountStore = .shared) -> Outcome {
        ...
        logger.notice("Resolved conversation \"\(conversation.id, privacy: .public)\"")
    ...
    static func outcome(forNoteID id: Int, in store: AccountStore = .shared) -> Outcome {
        ...
        logger.notice("Resolved note \(note.id, privacy: .public)")
        return .open(.page(target))
    }
    ...
    static func outcome(forCollectiveID id: Int, in store: AccountStore = .shared) -> Outcome {
        ...
        logger.notice("Resolved collective \(collective.id, privacy: .public)")
        return .open(.page(target))
    }
    ...
    ///
    /// This is the one resolution here that falls back rather than refusing, and the reason is that a page's address is the one route in this app not read out of the owning app's own routing — see `CollectivePageWebRoute`. Opening the collective is certainly the right neighbourhood, so a person who asked for a page and was shown its collective can see both what happened and where to go next, where opening nothing would look like the app had failed.
    static func outcome(forCollectivePageID id: Int, in store: AccountStore = .shared) -> Outcome {
        ...
        if let target = CollectivePageWebRoute.url(collectiveSlug: collective.slug, collectiveName: collective.name, page: page, on: serverAddress) {
            logger.notice("Resolved collective page \(page.id, privacy: .public)")
            return .open(.page(target))
        }
        ...
        logger.notice("No address for page \(page.id, privacy: .public); resolved its collective \(collective.id, privacy: .public) instead")
