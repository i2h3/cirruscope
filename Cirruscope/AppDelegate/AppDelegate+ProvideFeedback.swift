...
/// `AppDelegate`'s feedback support opens the user's default mail app with a draft addressed to Framecloud's feedback inbox, pre-filled with the environment details that help triage a report.
extension AppDelegate {

    /// `provideFeedback(_:)` opens the user's default mail app with a draft addressed to Framecloud's feedback inbox.
    ///
    ...
    func provideFeedback(_: Any?) {
        ...
        let subject = "Framecloud Feedback"
    ...
    ///
    /// It leaves an empty area at the top for the user to write in, then lists the macOS and Framecloud versions and appends the connected Nextcloud server version whenever `Settings.serverVersion` has one.
    private func feedbackBody() -> String {
        var details = [
            "macOS: \(ProcessInfo.processInfo.operatingSystemVersionString)",
            "Framecloud: \(appVersion)",
        ]
    ...
    /// `appVersion` is Framecloud's marketing version and build number read from the bundle, e.g. "1.0.0 (1)".
