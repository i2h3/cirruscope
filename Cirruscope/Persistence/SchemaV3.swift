...
///
/// It began identical in shape to `SchemaV2`, the version existing to give the freeze somewhere to hand off to rather than because anything about the store had changed, which is why `CirruscopeMigrationPlan` migrates across it with a lightweight stage. It has since gained `TalkConversation`, `ServerNote`, `ServerCollective` and `ServerCollectivePage`, and that stage is still lightweight: adding an entity, and adding a relationship to it that defaults to empty, are changes SwiftData infers on its own.
///
...
enum SchemaV3: VersionedSchema {
    ...
    static var models: [any PersistentModel.Type] {
        [Account.self, ServerApp.self, KeyboardShortcut.self, TalkConversation.self, ServerNote.self, ServerCollective.self, ServerCollectivePage.self]
    }
}
