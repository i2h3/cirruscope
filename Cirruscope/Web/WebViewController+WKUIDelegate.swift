...
extension WebViewController: WKUIDelegate {
    func webView(_: WKWebView, decideMediaCapturePermissionsFor origin: WKSecurityOrigin, initiatedBy _: WKFrameInfo, type _: WKMediaCaptureType) async -> WKPermissionDecision {
        logger.debug("Deciding media capture permission for origin \(origin.host) (WebViewController \(logID))")

        guard let serverHost = Settings.serverAddress?.host, origin.host.caseInsensitiveCompare(serverHost) == .orderedSame else {
            logger.debug("Origin is not the configured server; returning .prompt (WebViewController \(logID))")
            return .prompt
        }

        logger.debug("Origin is the configured server; granting media capture (WebViewController \(logID))")
        return .grant
    }
    ...
    func webView(_ webView: WKWebView, runOpenPanelWith parameters: WKOpenPanelParameters, initiatedByFrame _: WKFrameInfo) async -> [URL]? {
        logger.debug("Presenting file open panel (multiple selection: \(parameters.allowsMultipleSelection), directories: \(parameters.allowsDirectories)) (WebViewController \(logID))")
        ...
        if let window = webView.window {
            ...
            logger.debug("No host window; running the open panel modally (WebViewController \(logID))")
            response = panel.runModal()
        }

        guard response == .OK else {
            logger.debug("File open panel was cancelled; returning nil (WebViewController \(logID))")
            return nil
        }

        logger.debug("File open panel returned \(panel.urls.count) file(s) (WebViewController \(logID))")
        return panel.urls
    }
    ...
    func webView(_ webView: WKWebView, createWebViewWith _: WKWebViewConfiguration, for navigationAction: WKNavigationAction, windowFeatures _: WKWindowFeatures) -> WKWebView? {
        logger.debug("Handling new-window request for \(navigationAction.request.url?.absoluteString ?? "no URL") (WebViewController \(logID))")
        ...
        if navigationAction.targetFrame == nil {
            logger.debug("New-window request has no target frame; loading it in the existing web view (WebViewController \(logID))")
            webView.load(navigationAction.request)
        } else {
            logger.debug("New-window request already has a target frame; leaving it to that frame (WebViewController \(logID))")
        }

        logger.debug("Returning nil to decline a second web view (WebViewController \(logID))")
        return nil
    }
}
