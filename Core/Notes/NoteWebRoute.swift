...
enum NoteWebRoute {
    /// `appID` is the identifier of the Nextcloud app that owns a note, which is what its icon is cached under.
    ///
    /// Not a second fact but the same one written where it can be used: Nextcloud serves an app under `/apps/<app id>/`, so the `/apps/notes/note/<id>` route this builds names it outright. It is stated here rather than at each surface that needs it, because a surface guessing at it would be guessing at something this file already knows.
